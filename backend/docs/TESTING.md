...
Le workflow de développement local est Docker Compose depuis la racine du dépôt.

Le stack local comprend :

- `postgres` pour PostgreSQL ;
- `backend` pour FastAPI et les tests backend ;
- `frontend` pour Next.js et les vérifications frontend.

Les commandes de vérification doivent privilégier l'environnement conteneurisé afin d'éviter les dépendances implicites au host.
...
Les tests backend vivent sous `backend/tests/`. Les tests frontend transverses vivent sous `frontend/tests/`. Les migrations PostgreSQL vivent sous `db/migrations/`.
...
Les fichiers de migration vivent dans `db/migrations/` et la configuration Alembic dans `db/alembic.ini`.
