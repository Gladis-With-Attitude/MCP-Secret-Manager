...
`docs/PROJECT.md` defines the project vision, philosophy, goals, MVP scope and long-term direction.

It explains what MCP Secret Manager is and what it is not.

### Role of CONSTITUTION.md

`docs/CONSTITUTION.md` defines the principles that Pull Requests must not violate.

It is the highest-level governance document.

### Role of ARCHITECTURE.md

`docs/ARCHITECTURE.md` defines the internal architecture, layers, dependencies, modules, flows and invariants.

It guides where features should be added.

### Role of SECURITY.md

`docs/SECURITY.md` defines the official security strategy, threat model, security model, incident approach and production expectations.

It guides all security-sensitive changes.

### Role of DATABASE.md

`docs/DATABASE.md` defines the domain data model, entities, relationships, lifecycles and invariants.

It guides database design before migrations or ORM models exist.

### Role of CRYPTOGRAPHY.md

`docs/CRYPTOGRAPHY.md` defines the cryptographic strategy, key hierarchy, lifecycle and invariants.

It guides all cryptographic implementation decisions.

### Role of API_SPEC.md

`docs/API_SPEC.md` defines the REST API functional contract.

It guides endpoints, resources, errors, permissions and REST invariants.

### Role of MCP_SPEC.md

`docs/MCP_SPEC.md` defines the MCP interface contract.

It guides tools, permissions, responses, audit and agent-specific security.

### Role of TESTING.md

`docs/TESTING.md` defines the official testing and quality strategy.

It guides what must be tested before merge.

### Role of AI_RULES.md

`docs/AI_RULES.md` defines how AI assistants are allowed to contribute.

It guides AI-assisted analysis, implementation, review and documentation.

### Role of CONTRIBUTING.md

`docs/CONTRIBUTING.md` defines the contribution process for humans and assisted workflows.
