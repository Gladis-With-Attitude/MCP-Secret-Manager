...
type SecretVersionErrorViewProps = {
  error: unknown;
  onRetry?: () => void;
};

function SecretVersionErrorView({ error, onRetry }: SecretVersionErrorViewProps) {
  if (isApiError(error) && error.kind === "forbidden") {
    return <ForbiddenPage />;
  }

  const isNotFound = isApiError(error) && error.kind === "not_found";
  const title = isNotFound ? "Secret version not found" : "Unable to load secret versions";
