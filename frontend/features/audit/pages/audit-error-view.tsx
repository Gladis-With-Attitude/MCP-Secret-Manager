...
type AuditErrorViewProps = {
  error: unknown;
  onRetry?: () => void;
};

function AuditErrorView({ error, onRetry }: AuditErrorViewProps) {
  if (isApiError(error) && error.kind === "forbidden") {
    return <ForbiddenPage />;
  }

  const isNotFound = isApiError(error) && error.kind === "not_found";
  const title = isNotFound ? "Audit event not found" : "Unable to load audit logs";
