...
type SettingsErrorViewProps = {
  error: unknown;
  onRetry?: () => void;
};

function SettingsErrorView({ error, onRetry }: SettingsErrorViewProps) {
  if (isApiError(error) && error.kind === "forbidden") {
    return <ForbiddenPage />;
  }

  const isNotFound = isApiError(error) && error.kind === "not_found";
  const title = isNotFound ? "Settings not found" : "Unable to load settings";
