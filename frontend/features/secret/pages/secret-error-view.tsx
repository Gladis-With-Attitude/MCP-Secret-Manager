...
type SecretErrorViewProps = {
  error: unknown;
  onRetry?: () => void;
};

function SecretErrorView({ error, onRetry }: SecretErrorViewProps) {
  if (isApiError(error) && error.kind === "forbidden") {
    return <ForbiddenPage />;
  }

  const isNotFound = isApiError(error) && error.kind === "not_found";
  const title = isNotFound ? "Secret not found" : "Unable to load secrets";
